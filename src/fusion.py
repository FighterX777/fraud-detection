...
def predict_fraud(
    ...
    if transaction_features is not None:
        try:
            ...
            ml_mod = loaded_models.get("ml") if loaded_models else ml_model_path
            scaler = loaded_models.get("scaler") if loaded_models else "models/scaler.pkl"
            metrics = loaded_models.get("metrics") if loaded_models else "models/ml_metrics.json"
            
            ml_result = predict_transaction(transaction_features, model_path=ml_mod, scaler_path=scaler, metrics_path=metrics)
    ...
    if transaction_text is not None and transaction_text.strip():
        try:
            ...
            nlp_mod = loaded_models.get("nlp") if loaded_models else nlp_model_path
            nlp_result = predict_text(transaction_text, model_path=nlp_mod)
    ...
    if signature_paths is not None:
        try:
            ...
            cv_mod = loaded_models.get("cv") if loaded_models else cv_model_path
            cv_result = predict_signature(ref_path, test_path, model_path=cv_mod)
