...
def predict_transaction(features, model_path: str = MODEL_PATH,
    ...
    if isinstance(metrics_path, str):
        ...
        # If a float was passed instead of a path
        threshold = float(metrics_path) if metrics_path else 0.5
    ...
    elif scaler_path is not None and not isinstance(scaler_path, str):
        # We received a pre-loaded scaler object
        scaler = scaler_path
        try:
            if hasattr(scaler, "n_features_in_") and scaler.n_features_in_ == 2:
                scaled = scaler.transform([[features_copy[0], features_copy[29]]])[0]
